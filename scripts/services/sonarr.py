"""Sonarr configuration — root folders, download client, media management, tags."""
...
def configure_sonarr(
    ...
    """Configure a Sonarr instance (works for both sonarr and sonarr2).
...
def _ensure_download_client(client: ArrClient, config: dict, inst: dict) -> list[str]:
    """Ensure qBittorrent download client is configured."""
    changes = []
    ...
    if existing is None:
        ...
        for key, val in expected_fields.items():
            if key == "password":
                continue  # Password not returned by API
...
def _build_download_client_payload(fields: dict, inst: dict) -> dict:
    """Build the download client JSON payload for Sonarr."""
    return {
        "name": "qBittorrent",
        ...
        "fields": [
            ...
            {"name": "tvCategory", "value": fields["tvCategory"]},
...
def _set_media_management(client: ArrClient, config: dict) -> list[str]:
    ...
    # hardlinksCopyInstead: false means hardlinks ON
    ...
    # enableMediaInfo controls video analysis
    ...
    if updates:
        # Merge updates into current config and PUT back
